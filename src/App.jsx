...
/* =======================
   🔀 ROUTING SEÇENEĞİNİ SEÇ
   1) React Router v6  → Aşağıdaki satırı aç:
*/
import { useNavigate } from "react-router-dom";
/*
   2) Next.js          → Üsttekini sil, alttakini aç:
...
export default function WelcomeScreen() {
  ...
  const nextTickRef = useRef(0);          // bir sonraki çalınabilecek zaman (ms)
  const CLICK_INTERVAL = 180;             // ms — hızlanmayı engeller

  // 🔀 Router seç (KULLANDIĞINA GÖRE BİRİNİ AKTİF ET)
...
Hazırsan, oyun başlasın. 🧠💥`;
  ...
  const playKeySound = () => {
    ...
    if (now < nextTickRef.current) return; // metronom: zamanı değilse çalma
    if (!a.paused) return;                 // çalıyorsa üstüne bindirme

    try {
      a.volume = 0.06;
      a.playbackRate = 1.0;               // sabit hız — hızlanma hissini engeller
      a.currentTime = 0;
      a.play().catch(() => {});
    ...
    const startTimeout = setTimeout(() => {
      ...
      const interval = setInterval(() => {
        if (index < fullText.length) {
          ...
          stopKeySound();                      // yazı bitince sesi kesin durdur
  ...
  const handleStart = () => {
    // 🔀 Yönlendirme (kullandığına göre birini aktif bırak):
    navigate("/scenario");   // React Router v6
    // router.push("/scenario"); // Next.js
  };
