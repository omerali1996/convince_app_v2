...
def get_scenarios():
    ...
    for sid, scenario in scenarios.items():
        simplified_scenarios.append({
            "id": sid,
            "name": scenario.get("Senaryo Adı"),
            "story": scenario.get("Hikaye"),
            "goal": scenario.get("Amaç"),                 # <-- eklendi
            "system_prompt": scenario.get("System Prompt")
        })
...
def ask():
    ...
    try:
        messages = [{"role": "system", "content": scenario["System Prompt"]}]
        # geçmişi modele ekle
        for m in history:
